// BOD3D-TEST v11.43 — Sirrus and Tamara Lethal Blow
(function installLethalBlow(){
  ...
  function install(){
    ...
    function lethalAvailable(){return !!(combat&&state?.player&&state.player.flags?.special==='Lethal Blow'&&!state.player.flags.usedSpecial&&combat.pendingDiceRoll)}
    function resolveChoice(useLethal){
      if(!combat||!combat.pendingDiceRoll)return;
      combat.awaitingLethalDecision=false;combat.rolling=false;
    ...
    fightRound=function(){
      ...
      setTimeout(()=>{
        if(!combat)return;
        if(lethalAvailable()){
          combat.rolling=false;combat.awaitingLethalDecision=true;
          ...
          document.getElementById('combatLog').textContent='Your combat total is '+normalTotal+'. Choose Lethal Blow to double it to '+(normalTotal*2)+', or resolve normally.';
          renderCombat();
        }else originalResolveFightRound();
      },1050);
    };
    ...
    renderCombat=function(){
      ...
      if(state.player.flags?.special==='Lethal Blow'){
        ...
        note.textContent=state.player.flags.usedSpecial?'Lethal Blow used':'Press Fight — after the dice roll you may use Lethal Blow once.';
